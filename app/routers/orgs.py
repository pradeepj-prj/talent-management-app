...
async def get_org_skill_summary(
    org_unit_id: OrgId,
    conn: DbConn,
    limit: float = Query(default=20, ge=1, le=100, description="Number of top skills to return"),
):
    """**Endpoint 12a** — What are the top skills in this org unit (including child orgs)?"""
    return await svc.get_org_skill_summary(conn, org_unit_id, int(limit))
...
async def get_org_skill_experts(
    ...
    return await svc.get_org_skill_experts(conn, org_unit_id, int(skill_id), int(min_proficiency), int(limit))
