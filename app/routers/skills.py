...
async def get_top_experts(
    skill_id: int,
    conn: DbConn,
    min_proficiency: int = Query(default=4, ge=0, le=5, description="Minimum proficiency level"),
    limit: int = Query(default=20, ge=1, le=100, description="Max results to return"),
):
    """**Endpoint 3** — Who are the top experts in this skill?"""
    return await svc.get_top_experts(conn, skill_id, min_proficiency, limit)
...
async def get_skill_coverage(
    skill_id: int,
    conn: DbConn,
    min_proficiency: int = Query(default=3, ge=0, le=5, description="Minimum proficiency for coverage count"),
):
    """**Endpoint 4** — How many employees have this skill? Proficiency distribution histogram."""
    return await svc.get_skill_coverage(conn, skill_id, min_proficiency)
...
async def get_evidence_backed_candidates(
    ...
    min_evidence_strength: int = Query(default=4, ge=1, le=5, description="Minimum evidence signal strength"),
    ...
    return await svc.get_evidence_backed_candidates(conn, skill_id, min_proficiency, min_evidence_strength, limit)
...
async def get_stale_skills(
    skill_id: int,
    conn: DbConn,
    older_than_days: int = Query(default=365, ge=1, description="Skills not updated in this many days"),
):
    """**Endpoint 7** — Which employees have this skill but it hasn't been validated recently?"""
    return await svc.get_stale_skills(conn, skill_id, older_than_days)
...
async def get_cooccurring_skills(
    skill_id: int,
    conn: DbConn,
    min_proficiency: int = Query(default=3, ge=0, le=5, description="Minimum proficiency to consider"),
    top: int = Query(default=20, ge=1, le=50, description="Number of co-occurring skills to return"),
):
    """**Endpoint 9** — For employees strong in this skill, what other skills do they commonly have?"""
    return await svc.get_cooccurring_skills(conn, skill_id, min_proficiency, top)
